...
2
  ...
  try {
    ...
    // Unsubscribe group listener for removed member
    if (memberId === user?.uid && groupListenersRef.current[selectedGroup.id]) {
      groupListenersRef.current[selectedGroup.id](); // unsubscribe
      delete groupListenersRef.current[selectedGroup.id];
      setChatUser(null); // deselect chat
    }
